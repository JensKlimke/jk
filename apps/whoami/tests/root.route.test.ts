import express, { Express } from 'express';
import request from 'supertest';
import { WhoamiService } from '../src/services/whoami.service';
import { TemplateService } from '../src/services/template.service';
import { WhoamiController } from '../src/controllers/whoami.controller';
import { createRootRouter } from '../src/routes/root.route';

// Mock the services
jest.mock('../src/services/whoami.service');
jest.mock('../src/services/template.service');
jest.mock('../src/controllers/whoami.controller');

describe('Root Router', () => {
  let app: Express;
  let mockWhoamiService: jest.Mocked<WhoamiService>;
  let mockTemplateService: jest.Mocked<TemplateService>;
  let mockWhoamiController: jest.Mocked<WhoamiController>;
  const apiId = 'test-api-id';
  const whoamiInfo = {
    hostname: 'test-host',
    ips: ['192.168.1.1', '127.0.0.1'],
    remoteAddr: '10.0.0.1:12345',
    headers: {
      'user-agent': 'test-agent',
      'host': 'example.com'
    },
    apiId
  };

  beforeEach(() => {
    // Create mocks
    mockWhoamiService = new WhoamiService() as jest.Mocked<WhoamiService>;
    mockTemplateService = new TemplateService() as jest.Mocked<TemplateService>;
    mockWhoamiController = new WhoamiController(mockWhoamiService, mockTemplateService, apiId) as jest.Mocked<WhoamiController>;

    // Set up mock responses
    mockWhoamiService.getWhoamiInfo.mockReturnValue(whoamiInfo);
    mockTemplateService.renderWhoamiInfo.mockResolvedValue('<html><body>Test HTML</body></html>');

    // Mock controller methods
    mockWhoamiController.getWhoamiInfo = jest.fn().mockImplementation(async (req, res) => {
      const acceptHeader = req.headers.accept || '';
      const userAgent = req.headers['user-agent'] || '';

      if (acceptHeader.includes('application/json')) {
        res.json(whoamiInfo);
      } else if (acceptHeader.includes('text/html') || 
                userAgent.includes('Mozilla') || 
                userAgent.includes('Chrome') || 
                userAgent.includes('Safari')) {
        res.type('text/html').send('<html><body>Test HTML</body></html>');
        await mockTemplateService.renderWhoamiInfo(whoamiInfo);
      } else {
        let textResponse = `Hostname: ${whoamiInfo.hostname}\n`;
        textResponse += `IPs: ${whoamiInfo.ips.join(', ')}\n`;
        textResponse += `Remote Address: ${whoamiInfo.remoteAddr}\n`;
        textResponse += `App ID: ${whoamiInfo.apiId}\n\n`;
        textResponse += `Headers:\n`;

        Object.entries(whoamiInfo.headers).forEach(([key, value]) => {
          textResponse += `  ${key}: ${value}\n`;
        });

        res.type('text/plain').send(textResponse);
      }
    });

    // Create Express app
    app = express();
    app.use('/', createRootRouter(mockWhoamiController));
  });

  it('should return JSON when Accept header is application/json', async () => {
    const response = await request(app)
      .get('/')
      .set('Accept', 'application/json');

    expect(response.status).toBe(200);
    expect(response.type).toBe('application/json');
    expect(response.body).toEqual(whoamiInfo);
    expect(mockWhoamiController.getWhoamiInfo).toHaveBeenCalled();
  });

  it('should return HTML when Accept header is text/html', async () => {
    const response = await request(app)
      .get('/')
      .set('Accept', 'text/html');

    expect(response.status).toBe(200);
    expect(response.type).toBe('text/html');
    expect(response.text).toBe('<html><body>Test HTML</body></html>');
    expect(mockWhoamiController.getWhoamiInfo).toHaveBeenCalled();
  });

  it('should return HTML when User-Agent indicates a browser', async () => {
    const response = await request(app)
      .get('/')
      .set('User-Agent', 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36');

    expect(response.status).toBe(200);
    expect(response.type).toBe('text/html');
    expect(response.text).toBe('<html><body>Test HTML</body></html>');
    expect(mockWhoamiController.getWhoamiInfo).toHaveBeenCalled();
  });

  it('should return plain text by default', async () => {
    const response = await request(app)
      .get('/');

    expect(response.status).toBe(200);
    expect(response.type).toBe('text/plain');
    expect(response.text).toContain('Hostname: test-host');
    expect(response.text).toContain('IPs: 192.168.1.1, 127.0.0.1');
    expect(response.text).toContain('Remote Address: 10.0.0.1:12345');
    expect(response.text).toContain('App ID: test-api-id');
    expect(mockWhoamiController.getWhoamiInfo).toHaveBeenCalled();
  });

  it('should handle errors and return appropriate error response', async () => {
    // Create a new app and router for error tests
    const errorApp = express();

    // Mock controller to simulate error responses
    const errorController = new WhoamiController(mockWhoamiService, mockTemplateService, apiId) as jest.Mocked<WhoamiController>;
    errorController.getWhoamiInfo = jest.fn().mockImplementation((req, res) => {
      const acceptHeader = req.headers.accept || '';
      const userAgent = req.headers['user-agent'] || '';

      if (acceptHeader.includes('application/json')) {
        res.status(500).json({ error: 'Internal server error' });
      } else if (acceptHeader.includes('text/html') || 
                userAgent.includes('Mozilla') || 
                userAgent.includes('Chrome') || 
                userAgent.includes('Safari')) {
        res.status(500).type('text/html').send('<h1>Internal Server Error</h1><p>Something went wrong.</p>');
      } else {
        res.status(500).type('text/plain').send('Internal server error');
      }
    });

    errorApp.use('/', createRootRouter(errorController));

    // Test JSON error response
    const jsonResponse = await request(errorApp)
      .get('/')
      .set('Accept', 'application/json');

    expect(jsonResponse.status).toBe(500);
    expect(jsonResponse.type).toBe('application/json');
    expect(jsonResponse.body).toEqual({ error: 'Internal server error' });

    // Test HTML error response
    const htmlResponse = await request(errorApp)
      .get('/')
      .set('Accept', 'text/html');

    expect(htmlResponse.status).toBe(500);
    expect(htmlResponse.type).toBe('text/html');
    expect(htmlResponse.text).toContain('Internal Server Error');

    // Test plain text error response
    const textResponse = await request(errorApp)
      .get('/')
      .set('Accept', 'text/plain');

    expect(textResponse.status).toBe(500);
    expect(textResponse.type).toBe('text/plain');
    expect(textResponse.text).toBe('Internal server error');
  });
});
